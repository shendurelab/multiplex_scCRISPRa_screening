...
data_dir <- paste0('/net/shendure/vol10/www/content/members/CRISPRa_QTL_website/public/data/neuron_data/sequencing_data/cellranger_aggr_output/outs/count/filtered_feature_bc_matrix/')

##Making seurat object from FBC matrix
CRISPRaQTL_Pilot_data <- Read10X(data.dir = data_dir)
pilot <- CreateSeuratObject(counts = CRISPRaQTL_Pilot_data$`Gene Expression`, min.cells = 100,
	min.features = 200)

##Filtering for high quality cells
pilot[["percent.mt"]] <- PercentageFeatureSet(pilot, pattern = "^MT-")
pilot_subset <- NormalizeData(subset(pilot, subset = percent.mt < 17 &
	nCount_RNA > 1500))

##Reading in FBC matrix
matrix_dir <- paste0("/net/shendure/vol10/www/content/members/CRISPRa_QTL_website/public/data/neuron_data/sequencing_data/cellranger_aggr_output/outs/count/filtered_feature_bc_matrix/")
